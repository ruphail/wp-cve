...
if($_POST){
	 $xyz_cfm_hiddenSmtpId = intval($_POST['xyz_cfm_hiddenSmtpId']);
	if (
			! isset( $_REQUEST['_wpnonce'] )
			|| ! wp_verify_nonce( $_REQUEST['_wpnonce'], 'edit-smtp_'.$xyz_cfm_hiddenSmtpId )
	) {
...
if (($_POST['xyz_cfm_SmtpHostName']!= "") && ($_POST['xyz_cfm_SmtpEmailAddress'] != "") && ($_POST['xyz_cfm_SmtpPassword'] != "") && 
		($_POST['xyz_cfm_SmtpPortNumber']!= "") && ($_POST['xyz_cfm_SmtpSecuirity']!= "")){
			...
			$xyz_cfm_SmtpAuthentication = sanitize_text_field($_POST['xyz_cfm_SmtpAuthentication']);
			$xyz_cfm_SmtpHostName = sanitize_text_field($_POST['xyz_cfm_SmtpHostName']);
			$xyz_cfm_SmtpEmailAddress = sanitize_text_field($_POST['xyz_cfm_SmtpEmailAddress']);
			
			$xyz_cfm_SmtpPassword = $_POST['xyz_cfm_SmtpPassword'];
			$xyz_cfm_SmtpPortNumber = intval($_POST['xyz_cfm_SmtpPortNumber']);
			
			$xyz_cfm_SmtpSecuirity = sanitize_text_field($_POST['xyz_cfm_SmtpSecuirity']);
			$xyz_cfm_hiddenSmtpId = intval($_POST['xyz_cfm_hiddenSmtpId']);
			
			
			$blockedAccount_cfm = 0;
			
			if(isset($_POST['xyz_cfm_SmtpSetDefault']) && sanitize_text_field($_POST['xyz_cfm_SmtpSetDefault'])=="on"){
...
<h2>SMTP</h2>
	<form method="post">
	<?php wp_nonce_field( 'edit-smtp_'.$xyz_cfm_SmtpId );?>
	...
	<table class="widefat"  style="width:99%;">
			<tr valign="top">
				...
				<td><select name="xyz_cfm_SmtpAuthentication" id="xyz_cfm_SmtpAuthentication">
						<option value="true"
						<?php if(isset($_POST['xyz_cfm_SmtpAuthentication']) && sanitize_text_field($_POST['xyz_cfm_SmtpAuthentication'])=='true') { echo 'selected';}elseif($xyz_cfm_details->authentication =="true"){echo 'selected';} ?>>True</option>
						<option value="false"
						<?php if(isset($_POST['xyz_cfm_SmtpAuthentication']) && sanitize_text_field($_POST['xyz_cfm_SmtpAuthentication'])=='false') { echo 'selected';}elseif($xyz_cfm_details->authentication=="false"){echo 'selected';} ?>>False</option>
			...
			<tr valign="top">
				...
				<td ><input  name="xyz_cfm_SmtpSetDefault" type="checkbox"
					id="xyz_cfm_SmtpSetDefault" <?php if(isset($_POST['xyz_cfm_SmtpSetDefault']) && sanitize_text_field($_POST['xyz_cfm_SmtpSetDefault'])== "on" ){?>checked="checked"<?php }elseif($xyz_cfm_details->set_default == 1){?>checked="checked"<?php }?>/>
				</td>
			</tr>
			<tr>
