...
$xyz_cfm_array = array('contact-form-manager-manage-smtp','contact-form-manager-managecontactforms');

if ( is_admin() ){
	if(isset($_GET['page'])&&in_array($_GET['page'], $xyz_cfm_array)){
		ob_start();
	}
...
function cfm_menu(){
	...
	add_submenu_page('contact-form-manager-manage', 'Theme CSS', 'Theme CSS', 'manage_options', 'contact-form-manager-theme' ,'cfm_theme');
	add_submenu_page('contact-form-manager-manage', ' contact-form-manager-feature', 'Suggest a Feature', 'manage_options', 'contact-form-manager-feature' ,'cfm_suggestion_feature');
...
function cfm_suggestion_feature()
{
    require( dirname( __FILE__ ) . '/header.php' );
...
// 		$css_cfm_url = 'http://ajax.googleapis.com/ajax/libs/jqueryui/1.8.2/themes/smoothness/jquery-ui.css';
	...
	wp_register_style('xyz_jquery_ui_style', plugins_url('contact-form-manager/css/jquery-ui.css'));
