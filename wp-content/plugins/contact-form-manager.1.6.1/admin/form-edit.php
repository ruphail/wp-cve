...
if (isset($_POST) && isset($_POST['addSubmit'])) {
    ...
    if (! isset($_REQUEST['_wpnonce']) || ! wp_verify_nonce($_REQUEST['_wpnonce'], 'edit-form_' . $xyz_cfm_formId)) {
        
        wp_nonce_ays('edit-form_' . $xyz_cfm_formId);
        ...
        $xyz_cfm_formName = sanitize_text_field($_POST['formName']);
        $xyz_cfm_generatedCode = $_POST['generatedCode'];
        
        $xyz_cfm_to = sanitize_text_field($_POST['to']);
        if (get_option('xyz_cfm_sendViaSmtp') == 1) {
            $xyz_cfm_from_email_id = sanitize_text_field($_POST['xyz_cfm_senderEmailIdSmtp']); // from email id
        } else {
            $xyz_cfm_from_email_id = 0; // if smtp is off, email id set to 0
        }
        
        $xyz_cfm_from_email = sanitize_text_field($_POST['from']); // from email address
        
        $xyz_cfm_subject = sanitize_text_field($_POST['subject']);
        
        $xyz_cfm_formId = intval($_POST['formId']);
        // $xyz_cfm_submitMode = $_POST['submitMode'];
        $xyz_cfm_submitMode = 2;
        $xyz_cfm_cc = sanitize_text_field($_POST['cc']);
        $xyz_cfm_bcc = sanitize_text_field($_POST['bcc']);
        $xyz_cfm_mailType = intval($_POST['mailType']);
        ...
        $xyz_cfm_subjectReplay = sanitize_text_field($_POST['subjectReplay']);
        $xyz_cfm_mailTypeReplay = intval($_POST['mailTypeReplay']);
        ...
        $xyz_cfm_senderName = sanitize_text_field($_POST['senderName']);
        $xyz_cfm_replaySenderName = sanitize_text_field($_POST['replaySenderName']);
        $xyz_cfm_replaySenderEmailId = 0;
        if (isset($_POST['xyz_cfm_replaySenderEmailId'])) {
            $xyz_cfm_replaySenderEmailId = intval($_POST['xyz_cfm_replaySenderEmailId']); // reply email id
        }
        $xyz_cfm_replaySenderEmail = sanitize_text_field($_POST['replaySenderEmail']); // reply email address
        
        $xyz_cfm_enableReply = intval($_POST['enableReply']);
        
        $xyz_cfm_toEmailReply = sanitize_text_field($_POST['toEmailReply']);
        
        $xyz_cfm_redirectionLink = sanitize_text_field($_POST['redirectionLink']);
        
        $xyz_cfm_redispalyOption = sanitize_text_field($_POST['redisplayOption']);
            ...
            if ($_POST['newsletterSubscription'] == 1) {
                ...
                if ($_POST['newsletterSubscriptionVersion'] == 'std') {
                    if ($_POST['newsletterEmailShortcodeStd'] != '') {
                        $emailShortcode = sanitize_text_field($_POST['newsletterEmailShortcodeStd']);
                    } else {
                        $newsletterSubscriptionErrorFlag = 1;
                    }
                    
                    $nameShortcode = explode(',', sanitize_text_field($_POST['newsletterNameShortcodeStd']));
                    $customFields = serialize($nameShortcode);
                    
                    $emailListId = 1;
                    if ($newsletterSubscriptionErrorFlag == 0) {
                        $newsletterSubscriptionCode = array(
                            'newsletter_email_shortcode' => $emailShortcode,
                            'newsletter_email_list' => $emailListId,
                            'newsletter_custom_fields' => $customFields,
                            'newsletter_optin_mode' => $newsletterOptinMode,
                            'newsletter_subscription_status' => 1
                        );
                    }
                }
                ...
                if ($_POST['newsletterSubscriptionVersion'] == 'pre') {
                    ...
                    if ($_POST['newsletterEmailShortcodePre'] != '') {
                        $emailShortcode = sanitize_text_field($_POST['newsletterEmailShortcodePre']);
                    } else {
                        $newsletterSubscriptionErrorFlag = 1;
                    }
                    
                    if (isset($_POST['newsletterEmailListId'])) {
//                         $emailListId = implode(",", sanitize_text_field($_POST['newsletterEmailListId']))
                    ...
                    foreach ($additionalFieldInfoDetails as $InfoDetails) {
                        if ($_REQUEST['xyz_em_cfm_' . $InfoDetails->id] != '') {
                            ...
                            $fieldValue[$InfoDetails->id] = sanitize_text_field($_REQUEST['xyz_em_cfm_' . $InfoDetails->id]);
                        }
                    }
  ...
  $xyz_cfm_themename = intval($_POST['themename']);
        
        
        
        if (($newsletterSubscriptionErrorFlag == 0) && $xyz_cfm_toEmailReply != "" && $xyz_cfm_formName != "" && $xyz_cfm_generatedCode != "" && $xyz_cfm_to != "" && ($senderErrorFlag == 0) && $xyz_cfm_subject != "" && $xyz_cfm_mailBody != "" && $xyz_cfm_subjectReplay != "" && $xyz_cfm_mailBodyReplay != "") {
            ...
            if (empty($element_count)) {
                ...
                $wpdb->update($wpdb->prefix . 'xyz_cfm_form', array(
                    ...
                    'redisplay_option' => $xyz_cfm_redispalyOption,
                    'theme_id'=>$xyz_cfm_themename
                ...
                if (($newsletterSubscriptionCode != '') && ($newsletterActiveFlag == 1)) {
                    $wpdb->update($wpdb->prefix . 'xyz_cfm_form', $newsletterSubscriptionCode, array(
                        'id' => $xyz_cfm_formId
                    ));
                }
...
<div class="system_notice_area_style1" id="system_notice_area">
	Contact form successfully updated. &nbsp;&nbsp;&nbsp;<span
...
<div class="system_notice_area_style0" id="system_notice_area">
	Contact form already exists. &nbsp;&nbsp;&nbsp;<span
		id="system_notice_area_dismiss">Dismiss</span>
</div>
<?php
            }
            ...
            ?>
<div class="system_notice_area_style0" id="system_notice_area">
	Fill all mandatory fields. &nbsp;&nbsp;&nbsp;<span
		id="system_notice_area_dismiss">Dismiss</span>
</div>
<?php
        }
...
jQuery(document).ready(function() {
	...
	jQuery('#textFieldButton1').click(function() {
		...
		if(jQuery('#required1').prop('checked') == true){
			required = 1;
		}else{
			required = 0;
		}
		...
		var insert_nonce  = '<?php echo wp_create_nonce('insert_elements');?>';
		var dataString = { 
	...
	jQuery('#textFieldButton2').click(function() {
		...
		if(jQuery('#required2').prop('checked') == true){
			required = 1;
		}else{
			required = 0;
		}
	...
	jQuery('#textFieldButton3').click(function() {
		...
		if(jQuery('#required3').prop('checked') == true){
			required = 1;
		}else{
			required = 0;
		}
	...
	jQuery('#textFieldButton4').click(function() {
		...
		if(jQuery('#required4').prop('checked') == true){
			required = 1;
		}else{
			required = 0;
		}

		if(jQuery('#viewMultipleSelectDropDown').prop('checked') == true){
			multipleSelect = 1;
		}else{
			multipleSelect = 0;
		}
	...
	jQuery('#textFieldButton5').click(function() {
		...
		if(jQuery('#required5').prop('checked') == true){
			required = 1;
		}else{
			required = 0;
		}
	...
	jQuery('#textFieldButton6').click(function() {
		...
		var required ='';
		var singleLineView = '';
		if(jQuery('#required6').prop('checked') == true){
			required = 1;
		}else{
			required = 0;
		}

		if(jQuery('#viewCheckboxOptions').prop('checked') == true){
			singleLineView = 1;
		}else{
			singleLineView = 0;
		}
	...
	jQuery('#textFieldButton7').click(function() {
		...
		if(jQuery('#required7').prop('checked') == true){
			required = 1;
		}else{
			required = 0;
		}

		if(jQuery('#viewRadiobuttonOptions').prop('checked') == true){
			singleLineView = 1;
		}else{
			singleLineView = 0;
		}
	...
	jQuery('#textFieldButton8').click(function() {
		...
		if(jQuery('#required8').prop('checked') == true){
			required = 1;
		}else{
			required = 0;
		}
	...
	jQuery('#textFieldButton10').click(function() {
		...
		if(jQuery('#reCaptcha').prop('checked') == true){
				var reCaptcha = 1;
		}
		else{
				var reCaptcha = 0;
...
$theme_details = $wpdb->get_results( "SELECT * FROM ".$wpdb->prefix."xyz_cfm_theme_details  ORDER BY id  " );
...
<div>
	...
	<fieldset
		...
		<?php wp_nonce_field( 'edit-form_'.$formDetails->id );?>
			<input type="hidden" id="formId" name="formId"
				value="<?php if(isset($_POST['formId'])){ echo esc_html($_POST['formId']);}else{ echo esc_html($formDetails->id); }?>">
			<div>
				<table
					style="width: 98%; background-color: #F9F9F9; border: 1px solid #E4E4E4; border-width: 1px; margin-left: 10px; margin-right: 10px;">
					<tr valign="top">
						<td style="border-bottom: none;"><div
								style="width: 200px; float: left;">
								<span style="color: red;">*</span>Form Name
							</div> <input type="text" name="formName" id="formName"
        ...
        ?>
			...
			<table
				style="width: 98%; margin-left: 10px; border: 1px solid #E4E4E4;">
				<tr>
					<td style="width: 50%;">
						<table
        ...
        if (isset($_POST['generatedCode'])) {
            ...
            wp_editor($xyz_cfm_pageCode, 'generatedCode');
        } else {
        ...
        ?></td>
					...
					<td style="width: 50%; vertical-align: text-top;">
						<table
							...
							<tr>
								<td style="padding-bottom: 15px; padding-top: 12px;"
									id="bottomBorderNone"><span style="font-size: 22px;">Form
										Elements</span>
												...
												<table class="tableStyle">
													...
													<tr>
														...
														<td><input type="text" class="xyz_cfm_NoEnterSubmit"
															name="elementName1" id="elementName1"><span
													...
													<tr>
														...
														<td><input type="text" class="xyz_cfm_NoEnterSubmit"
															name="className1" id="className1"></td>
													</tr>
													<tr>
														<td>Maxlength(optional)</td>
														<td><input type="text" class="xyz_cfm_NoEnterSubmit"
															name="maxlength1" id="maxlength1"></td>
													</tr>
													...
													<tr>
														...
														<td><input class="xyz_cfm_NoEnterSubmit" type="text"
															name="defaultValue1" id="defaultValue1"></td>
													...
													<tr>
														<td id="bottomBorderNone"><input
															...
															type="button" name="textFieldButton1" value="Get Code"></td>
											...
											<div id="emailField" style="display: none;">
												...
												<table class="tableStyle">
													...
													<tr>
														...
														<td>Form Element Name</td>
														<td><input type="text" class="xyz_cfm_NoEnterSubmit"
															name="elementName2" id="elementName2"><span
															style="color: red;">*</span></td>
													...
													<tr>
														...
														<td>Style Class Name (optional)</td>
														<td><input type="text" class="xyz_cfm_NoEnterSubmit"
															name="className2" id="className2"></td>
													</tr>
													<tr>
														<td>Maxlength (optional)</td>
														<td><input type="text" class="xyz_cfm_NoEnterSubmit"
															name="maxlength2" id="maxlength2"></td>
													</tr>
													...
													<tr>
														<td>Default value (optional)</td>
														<td><input class="xyz_cfm_NoEnterSubmit" type="text"
															name="defaultValue2" id="defaultValue2"></td>
													...
													<tr>
														<td id="bottomBorderNone"><input
															...
															type="button" name="textFieldButton2" value="Get Code"></td>
													</tr>
											...
											<div id="textArea" style="display: none;">
												...
												<table class="tableStyle">
													...
													<tr>
														...
														<td>Form Element Name</td>
														<td><input type="text" class="xyz_cfm_NoEnterSubmit"
															name="elementName3" id="elementName3"><span
															style="color: red;">*</span></td>
													...
													<tr>
														...
														<td>Style Class Name (optional)</td>
														<td><input type="text" class="xyz_cfm_NoEnterSubmit"
															name="className3" id="className3"></td>
													</tr>

													<tr>
														<td>Default value (optional)</td>
														<td><input class="xyz_cfm_NoEnterSubmit" type="text"
															name="defaultValue3" id="defaultValue3"></td>
													</tr>

													<tr>
														<td>Cols (optional)</td>
														<td><input type="text" name="colLength3"
															class="xyz_cfm_NoEnterSubmit" id="colLength3"></td>
													</tr>

													<tr>
														<td>Rows (optional)</td>
														<td><input type="text" name="rowLength3"
															class="xyz_cfm_NoEnterSubmit" id="rowLength3"></td>
													</tr>
													...
													<tr>
														<td id="bottomBorderNone"><input
															...
															type="button" name="textFieldButton3" value="Get Code"></td>
													</tr>
											...
											<div id="dropDownMenu" style="display: none;">
												...
												<table class="tableStyle">
													...
													<tr>
														...
														<td>Form Element Name</td>
														<td><input type="text" class="xyz_cfm_NoEnterSubmit"
															name="elementName4" id="elementName4"><span
															style="color: red;">*</span></td>
													</tr>
													<tr>
														...
														<td><input type="text" class="xyz_cfm_NoEnterSubmit"
															...
															style="color: red;">*</span> <br /> <b>Example 1</b> :
													...
													<tr>
														...
														<td><input type="text" class="xyz_cfm_NoEnterSubmit"
															...
															id="dropDownOptions4DefaultValue"></td>
													</tr>
													<tr>
														<td>Style Class Name (optional)</td>
														<td><input type="text" class="xyz_cfm_NoEnterSubmit"
															name="className4" id="className4"></td>
													</tr>
													<tr>
														<td id="bottomBorderNone"><input
															class="button-primary cfm_bottonWidth"
															id="textFieldButton4" style="cursor: pointer;"
															type="button" name="textFieldButton4" value="Get Code"></td>
													</tr>
											...
											<div id="dateField" style="display: none;">
												...
												<table class="tableStyle">
													...
													<tr>
														...
														<td>Form Element Name</td>
														<td><input type="text" class="xyz_cfm_NoEnterSubmit"
															name="elementName5" id="elementName5"><span
															style="color: red;">*</span></td>
													</tr>
													<tr>
														<td>Style Class Name (optional)</td>
														<td><input class="xyz_cfm_NoEnterSubmit" type="text"
															name="className5" id="className5"></td>
													</tr>
													<tr>
														<td id="bottomBorderNone"><input
															class="button-primary cfm_bottonWidth"
															id="textFieldButton5" style="cursor: pointer;"
															type="button" name="textFieldButton5" value="Get Code"></td>
													</tr>
											...
											<div id="checkBoxes" style="display: none;">
												...
												<table class="tableStyle">
													...
													<tr>
														...
														<td>Form Element Name</td>
														<td><input type="text" class="xyz_cfm_NoEnterSubmit"
															name="elementName6" id="elementName6"><span
															style="color: red;">*</span></td>
													</tr>
													<tr>
														...
														<td><input type="text" class="xyz_cfm_NoEnterSubmit"
															...
															style="color: red;">*</span> <br /> Please use comma(,)
															to separate option values.<br /> <b>Example 1</b> :
															a,b,c,d <br /> <b>Example 2</b> : a=>1,b=>2,c=>3,d=>4</td>
													</tr>
													<tr>
														...
														<td><input type="text" class="xyz_cfm_NoEnterSubmit"
															...
															id="checkBoxOptions6DefaultValue"></td>
													</tr>
													<tr>
														<td colspan="2"><input type="checkbox"
													...
													<tr>
														<td>Style Class Name (optional)</td>
														<td><input type="text" class="xyz_cfm_NoEnterSubmit"
															name="className6" id="className6"></td>
													</tr>
													<tr>
														<td id="bottomBorderNone"><input
															class="button-primary cfm_bottonWidth"
															id="textFieldButton6" style="cursor: pointer;"
															type="button" name="textFieldButton6" value="Get Code"></td>
													</tr>
											...
											<div id="radioButtons" style="display: none;">
												...
												<table class="tableStyle">
													...
													<tr>
														...
														<td>Form Element Name</td>
														<td><input type="text" class="xyz_cfm_NoEnterSubmit"
															name="elementName7" id="elementName7"><span
															style="color: red;">*</span></td>
													...
													<tr>
														<td>Options</td>
														<td><input type="text" class="xyz_cfm_NoEnterSubmit"
															name="radioOptions7" id="radioOptions7"><span
															style="color: red;">*</span> <br /> <b>Example 1</b> :
															a,b,c,d <br /> <b>Example 2</b> : a=>1,b=>2,c=>3,d=>4</td>
													</tr>
													<tr>
														...
														<td><input type="text" class="xyz_cfm_NoEnterSubmit"
															...
															id="radioOptions7DefaultValue"></td>
													</tr>
													<tr>
														<td colspan="2"><input type="checkbox"
													...
													<tr>
														<td>Style Class Name (optional)</td>
														<td><input type="text" class="xyz_cfm_NoEnterSubmit"
															name="className7" id="className7"></td>
													</tr>
													<tr>
														<td id="bottomBorderNone"><input
															class="button-primary cfm_bottonWidth"
															id="textFieldButton7" style="cursor: pointer;"
															type="button" name="textFieldButton7" value="Get Code"></td>
													</tr>
											...
											<div id="fileUpload" style="display: none;">
												...
												<table class="tableStyle">
													...
													<tr>
														...
														<td>Form Element Name</td>
														<td><input type="text" class="xyz_cfm_NoEnterSubmit"
															name="elementName8" id="elementName8"><span
															style="color: red;">*</span></td>
													</tr>
													...
													<tr>
														...
														<td><input class="xyz_cfm_NoEnterSubmit" type="text"
															name="fileSize8" id="fileSize8"></td>
													</tr>
													<tr>
														<td>Style Class Name (optional)</td>
														<td><input class="xyz_cfm_NoEnterSubmit" type="text"
															name="className8" id="className8"></td>
													...
													<tr>
														<td id="bottomBorderNone"><input
															...
															type="button" name="textFieldButton8" value="Get Code"></td>
													</tr>
											...
											<div id="submitButton" style="display: none;">
												<table class="tableStyle">
													...
													<tr>
														<td>Form Element Name</td>
														<td><input type="text" class="xyz_cfm_NoEnterSubmit"
															name="elementName9" id="elementName9"><span
															style="color: red;">*</span></td>
													</tr>
													<tr>
														<td>Display Name</td>
													...
													<tr>
														...
														<td>Style Class Name (optional)</td>
														<td><input class="xyz_cfm_NoEnterSubmit" type="text"
															name="className9" id="className9"></td>
													</tr>
													<tr>
														<td id="bottomBorderNone"><input
															class="button-primary cfm_bottonWidth"
															id="textFieldButton9" style="cursor: pointer;"
															type="button" name="values" value="Get Code"></td>
													</tr>
						...
						<table class="widefat"
							...
							<tr>
								<td><b>Options after form submission</b></td>
							</tr>
							<tr>
								<td style="border-bottom: none;">
									...
									<div style="width: 100%;">
										<br> Redisplay empty form after submission: <input style=""
									...
									<div style="width: 100%;">
										<br> <strong>NOTE: Both settings will not work together</strong>
									</div> <br>
								</td>
							</tr>
				...
				<tr>
					<td colspan="2">
						<table
							style="width: 99%; background-color: #F9F9F9; border: 1px solid #E4E4E4; border-width: 1px; float: left;">
							<tr>
								<td style="border-bottom: 1px solid #F9F9F9;"><span
									style="font-size: 22px;">Theme Settings</span></td>
							</tr>

							<tr>
								<td style="border-bottom: 1px solid #F9F9F9;">
								<div class="xyz_cfm_theme_bar" style="min-height: 162px;">
                                   <p>
			<img style="height:100px;" src ="data:image/gif;base64,R0lGODlhAQABAIAAAAAAAP///yH5BAEAAAAALAAAAAABAAEAAAIBRAA7">
						</p>
						<p>
<input  type="radio" name="themename" value="0" <?php if($formDetails->theme_id==0){echo 'checked';} ?>> None
								</p>
								</div>	
								<?php 
								...
								foreach( $theme_details as $entry ) {
								    ...
								    $class="";
								    if($entry->product_edition== 1)
								       $class="class='checked_false'";
                                   ?>
                                   <div class="xyz_cfm_theme_bar">
                                   <p>
								<img style="height:100px;" src="<?php echo plugins_url('contact-form-manager/images/theme/'.$previewimg)?>">
						</p>
						<p>
								<input <?php echo $class ;?> type="radio" name="themename" value="<?php echo $entry->id ?>" <?php if($entry->id ==$formDetails->theme_id){echo 'checked';} ?>> <?php echo $entry->name;?>
								</p></div>
								<?php } ?>
</td>
							</tr>
				...
				<tr>
					<td>
						<table
							...
							<tr>
								<td style="border-bottom: 1px solid #F9F9F9;"><span
									style="font-size: 22px;">Mail to site admin</span></td>
							</tr>

							<tr>
								<td><span style="color: red;">*</span>To Email&nbsp;:&nbsp;<br>
									<input style="width: 350px;" type="text" name="to" id="to"
							...
							<tr>
								<td>BCC&nbsp;:&nbsp;<br> <input style="width: 350px;"
    ...
    if (get_option('xyz_cfm_sendViaSmtp') == 1) {
        ?>
							<tr>
								<td><span style="color: red;">*</span> SMTP Account&nbsp;:&nbsp;<br> <?php
        ...
        foreach ($xyz_cfm_getSenderdetails as $xyz_cfm_getSender) {
            ?>
										<option value="<?php echo $xyz_cfm_getSender->id;?>"
											...
											<?php echo esc_html($xyz_cfm_getSender->user);?>
										</option>
										<?php
        }
        ?>
    ...
    ?>
        ...
        ?>
								...
								From Email&nbsp;:&nbsp;<br> <input style="width: 350px;"
							...
							<tr valign="top">
								<td style="width: 45%; border-bottom: none;"><span
									style="color: red;">*</span>Mail Body&nbsp;:&nbsp;<br>
									<div id="htmlMail">

										<?php
        if (isset($_POST['mailBodyEditor'])) {
            $xyz_cfm_mailBody = $_POST['mailBodyEditor'];
            wp_editor($xyz_cfm_mailBody, 'mailBodyEditor');
        } else {
...
if (isset($_POST['mailBodyPlainText'])) {
            echo esc_textarea($_POST['mailBodyPlainText']);
        } else {
            echo esc_textarea($formDetails->mail_body);
        }
        ?>
										</textarea>
									</div></td>
					...
					<td>
						<table
							...
							<tr>
								<td style="border-bottom: 1px solid #F9F9F9;"><span
									style="font-size: 22px;">Auto-reply</span></td>
							</tr>
							<tr>
								<td style="border-bottom: none;"><div
        ...
        ?>
							<tr>
								<td>Reply Sender Email&nbsp;:&nbsp;<br> <input
							...
							<tr>
								<td><span style="color: red;">*</span>To Email&nbsp;:&nbsp;<br>
									<input style="width: 350px;" type="text" name="toEmailReply"
									id="toEmailReply"
        ...
        if (isset($_POST['mailBodyReplayEditor'])) {
            $xyz_cfm_mailBodyReplay = $_POST['mailBodyReplayEditor'];
            wp_editor($xyz_cfm_mailBodyReplay, 'mailBodyReplayEditor');
        } else {
...
if (isset($_POST['mailBodyReplayPlainText'])) {
    ...
    if (($newsletterStandardActiveFlag == 1) || ($newsletterPremiumActiveFlag == 1)) {
        ?>	
							<input type="hidden" name="newsletterSubscriptionVersion"
					value="<?php echo $pluginType;?>">

				<table class="widefat"
					style="width: 98%; background-color: #F9F9F9; border: 1px solid #E4E4E4; border-width: 1px; margin-left: 10px; margin-right: 10px;">
					<tr>
						<td colspan="3"
							style="border-bottom: 1px solid #F9F9F9; padding-top: 20px; padding-bottom: 20px;"><span
							style="font-size: 22px;">XYZ Newsletter Subscription Settings</span>
							<br />
						<b>Requires XYZ Newsletter
									<?php
        if ($pluginType == 'std') {
            echo ' Version - 1.2.2 or higher ';
        } elseif ($pluginType == 'pre') {
            echo ' Version - 1.0.2 or higher ';
        }
        ?>
									</b></td>
					</tr>
						...
						<td style="border: none;">
							<div style="width: 100%;">
								<input style="" type="radio" name="newsletterSubscription"
									...
									checked="checked" <?php }?>>Yes <input style="" type="radio"
									name="newsletterSubscription" value="2"
...
jQuery(document).ready(function() {
//jQuery('#user').show();
jQuery("input:radio[name=newsletterSubscription]").click(function() {
 var newsletterSubscriptionRadio = jQuery(this).val();
 if(newsletterSubscriptionRadio == 1){
jQuery('#user').show();jQuery('#user1').show();jQuery('#user2').show();
}
else{ jQuery('#user').hide();jQuery('#user1').hide();jQuery('#user2').hide(); }
});
					...
					<tr id="user">
						...
						<td style="border: none;">&nbsp;:&nbsp;</td>
						<td style="border: none;"><select name="xyz_newsletterOptinMode">
								<option value='Active'
									<?php if(isset($_POST['xyz_newsletterOptinMode']) && ($_POST['xyz_newsletterOptinMode'] == 'Active') ){?>
									selected="selected"
									<?php }else{if($formDetails->newsletter_optin_mode == 'Active'){?>
									selected="selected" <?php }}?>>Single Optin</option>
								<option value='Pending'
									<?php if(isset($_POST['xyz_newsletterOptinMode']) && ($_POST['xyz_newsletterOptinMode'] == 'Pending') ){?>
									selected="selected"
									<?php }else{if($formDetails->newsletter_optin_mode == 'Pending'){?>
									selected="selected" <?php }}?>>Double Optin</option>
						</select></td>
					</tr>
								<?php
        ...
        if ($newsletterStandardActiveFlag == 1) {
            ...
            ?>
					...
					<tr id="user1">
						<td style="border: none;">Email field short code <span
							style="color: red;">*</span></td>
						<td style="border: none;">&nbsp;:&nbsp;</td>
						<td style="border: none;"><input style="width: 350px;" type="text"
							name="newsletterEmailShortcodeStd"
							id="newsletterEmailShortcodeStd"
					...
					<tr id="user2">
						...
						<td style="border: none;">&nbsp;:&nbsp;</td>
						<td style="border: none;"><input style="width: 350px;" type="text"
							name="newsletterNameShortcodeStd" id="newsletterNameShortcodeStd"
							value="<?php
            if (isset($_POST['newsletterNameShortcodeStd'])) {
                ...
                if (count($fieldArray) == 1) {
                    ...
                    echo esc_html(implode(',', $fieldArray));
                }
            }
            ?>"></td>
					</tr>
        ...
        if ($newsletterPremiumActiveFlag == 1) {
            ...
            if (!empty($xyz_em_getEmailListdetails)) {
                ...
                ?>
					...
					<tr>
						<td style="border: none; width: 250px;">Select the email list <span
							style="color: red;">*</span></td>
						<td style="border: none;">&nbsp;:&nbsp;</td>
						<td style="border: none;"><select name="newsletterEmailListId[]"
							multiple>
											<?php
                foreach ($xyz_em_getEmailListdetails as $xyz_em_getEmailList) {
                    ?>
											<option value="<?php echo $xyz_em_getEmailList->id;?>"
									<?php if(isset($_POST['newsletterEmailListId']) && in_array($xyz_em_getEmailList->id,$_POST['newsletterEmailListId'])){?>
									selected='selected'
									<?php }else{if(in_array($xyz_em_getEmailList->id,explode(",",$formDetails->newsletter_email_list))){?>
									selected='selected' <?php }}?>><?php echo $xyz_em_getEmailList->name;?></option>
											<?php
                }
                ?>
										</select></td>
					...
					<tr>
						...
						<td style="border: none;"><input type="text"
							name="newsletterEmailShortcodePre"
							id="newsletterEmailShortcodePre"
                ...
                foreach ($additionalFieldInfoDetails as $infoDetails) {
                    ?>
						...
						<td style="border: none;"><label
							for="xyz_em_<?php echo $infoDetails->field_name;?>"><?php echo $infoDetails->field_name;?> - short code</label>
						</td>
						<td style="border: none;">&nbsp;:&nbsp;</td>
						<td style="border: none;"><input
							name="xyz_em_cfm_<?php echo $infoDetails->id;?>" type="text"
							id="xyz_em_cfm_<?php echo $infoDetails->id;?>"
							value="<?php
                    if (isset($_POST['xyz_em_cfm_' . $infoDetails->id])) {
                        ...
                        if (isset($fieldArray[$infoDetails->id])) {
                            echo esc_attr($fieldArray[$infoDetails->id]);
                        }
                    }
                    ?>" /></td>
					</tr>
					...
					<tr>
						<td colspan="3"
							...
							<span style="font-size: 22px;">XYZ Newsletter Subscription
								Setting</span> <br /> <b>Requires Newsletter Manager Version -
								1.2.2 or higher / XYZ WP Newsletter Version 1.0.2 or higher </b>
						</td>
					</tr>
				</table> 
						<?php
    ...
    ?>	
			...
			<div style="margin: 5px; width: 100%;">
				<input class="submit_cfm"
					style="color: #FFFFFF; border-radius: 4px; border: 1px solid #1A87B9;padding:10px;"
...
<div id="xyz_cfm_premium_only_info">
	<label>Only available in Premium version</label>
</div>
<script type="text/javascript">

jQuery(".checked_false").click(function(e){
	premiumClick(e);
...
jQuery("#xyz_cfm_premium_only_info").css({"left" : left + "px", "top" : top + "px"});
jQuery("#xyz_cfm_premium_only_info").show();
jQuery("#xyz_cfm_premium_only_info").fadeIn( "slow", function() {
window.tooltip = setTimeout(function(){ jQuery("#xyz_cfm_premium_only_info").hide(); }, 2000);
});
return false;
}
</script>
<style>
#xyz_cfm_premium_only_info
{
display:none;
